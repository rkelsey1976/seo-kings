# SEO Kings – Target Keywords Across All Areas

Master list of keywords we target on the site, area pages, GBP, and content. Use for on-page SEO, meta, headings, and content planning.

---

## 1. Brand & service keywords (site-wide)

**Primary**
- local SEO Bath
- local SEO BANES
- SEO agency Bath
- SEO agency BANES
- Google Business Profile Bath
- Google Business Profile BANES
- local SEO agency Bath
- local SEO agency BANES

**Secondary**
- free SEO audit Bath
- free SEO audit BANES
- SEO for plumbers
- SEO for electricians
- SEO for builders
- SEO for trades
- map pack SEO
- get found on Google Bath
- get found on Google BANES
- BANES SEO agency
- Bath SEO company

**Offer / CTA**
- free local SEO audit
- free Google Business Profile audit
- SEO for small business Bath
- SEO for tradespeople BANES

---

## 2. Areas we target

| Area | Postcode(s) | Notes |
|------|-------------|--------|
| Bath | BA1, BA2 | Primary; highest volume |
| Keynsham | BS31 | Bristol edge; BS31 |
| Midsomer Norton | BA3 | Somer Valley |
| Radstock | BA3 | Somer Valley |
| Peasedown St John | BA2 | South of Bath |
| Paulton | BS39 | Cam Valley |
| Saltford | BS31 | Between Bath & Bristol |
| Timsbury | BA2 | Rural village |
| BANES | — | Bath and North East Somerset (region) |

---

## 3. Trades we target (by category)

**Core trades (all areas)**
- plumber
- electrician
- builder
- roofer
- handyman
- decorator
- gardener
- cleaner

**High-intent / emergency**
- emergency plumber
- emergency builder
- boiler repair
- locksmith

**Specialist trades**
- bathroom fitter
- kitchen fitter
- landscaper
- garage / garage services

**Postcode variant**
- electrician BS31 (Keynsham / Saltford)
- plumber BS31
- plumber BS39 (Paulton)
- plumber BA2, BA3 (Bath / Timsbury / Radstock area)

---

## 4. Area + trade keywords (master list by area)

Use these for area pages, titles, and content. Format: **trade + area**.

### Bath
- plumber Bath
- electrician Bath
- builder Bath
- roofer Bath
- bathroom fitter Bath
- kitchen fitter Bath
- handyman Bath
- gardener Bath
- cleaner Bath
- locksmith Bath
- emergency plumber Bath
- boiler repair Bath

### Keynsham
- plumber Keynsham
- electrician Keynsham
- electrician BS31
- builder Keynsham
- emergency builder Keynsham
- locksmith Keynsham
- cleaner Keynsham
- gardener Keynsham
- decorator Keynsham
- roofer Keynsham

### Midsomer Norton
- plumber Midsomer Norton
- electrician Midsomer Norton
- builder Midsomer Norton
- roofer Midsomer Norton
- decorator Midsomer Norton
- handyman Midsomer Norton
- emergency plumber Midsomer Norton
- garage services Midsomer Norton
- boiler repair Midsomer Norton

### Radstock
- plumber Radstock
- electrician Radstock
- builder Radstock
- handyman Radstock
- garage Radstock
- decorator Radstock

### Peasedown St John
- plumber Peasedown
- electrician Peasedown
- builder Peasedown St John
- landscaper Peasedown
- handyman Peasedown
- cleaner Peasedown
- emergency plumber Peasedown
- roofer Peasedown

### Paulton
- plumber Paulton
- electrician Paulton
- builder Paulton
- cleaner Paulton
- gardener Paulton
- decorator Paulton
- handyman Paulton
- emergency plumber Paulton
- boiler repair Paulton

### Saltford
- plumber Saltford
- electrician Saltford
- builder Saltford
- decorator Saltford
- gardener Saltford
- cleaner Saltford
- handyman Saltford
- emergency plumber Saltford
- boiler repair Saltford

### Timsbury
- plumber Timsbury
- electrician Timsbury
- builder Timsbury
- handyman Timsbury
- gardener Timsbury
- decorator Timsbury
- emergency plumber Timsbury
- boiler repair Timsbury

---

## 5. Keywords by trade (across areas)

Use for service/trade-focused content or landing pages.

| Trade | Example area keywords |
|-------|------------------------|
| Plumber | plumber Bath, plumber Keynsham, plumber Midsomer Norton, plumber Radstock, plumber Peasedown, plumber Paulton, plumber Saltford, plumber Timsbury, emergency plumber [area], boiler repair [area] |
| Electrician | electrician Bath, electrician Keynsham, electrician BS31, electrician Midsomer Norton, electrician Radstock, electrician Peasedown, electrician Paulton, electrician Saltford, electrician Timsbury |
| Builder | builder Bath, builder Keynsham, builder Midsomer Norton, builder Radstock, builder Peasedown St John, builder Paulton, builder Saltford, builder Timsbury, emergency builder Keynsham |
| Roofer | roofer Bath, roofer Keynsham, roofer Midsomer Norton, roofer Peasedown, roofer Paulton (add if needed) |
| Handyman | handyman Bath, handyman Midsomer Norton, handyman Radstock, handyman Peasedown, handyman Paulton, handyman Saltford, handyman Timsbury |
| Decorator | decorator Keynsham, decorator Midsomer Norton, decorator Radstock, decorator Paulton, decorator Saltford, decorator Timsbury |
| Gardener | gardener Bath, gardener Keynsham, gardener Paulton, gardener Saltford, gardener Timsbury |
| Cleaner | cleaner Bath, cleaner Keynsham, cleaner Peasedown, cleaner Paulton, cleaner Saltford |
| Locksmith | locksmith Bath, locksmith Keynsham |
| Landscaper | landscaper Peasedown |
| Garage | garage Radstock, garage services Midsomer Norton |
| Bathroom fitter | bathroom fitter Bath |
| Kitchen fitter | kitchen fitter Bath |

---

## 6. Regional & postcode keywords

**Region**
- local SEO BANES
- SEO Bath and North East Somerset
- BANES trades
- Bath and BANES SEO
- Somer Valley SEO (Midsomer Norton, Radstock, Paulton)
- Bristol edge (Keynsham, Saltford)

**Postcodes (where useful)**
- SEO BS31 (Keynsham, Saltford)
- plumber BS31
- electrician BS31
- SEO BS39 (Paulton)
- SEO BA2, BA3 (Bath, Timsbury, Radstock, Midsomer Norton area)

---

## 7. Website design + location (for Web Design page)

Use these to target "website design [location]" searches. Add to Web Design page meta, H2 and body copy.

**Primary**
- website design Bath
- website design BANES
- web design Bath
- web design BANES

**Other BANES areas**
- website design Keynsham
- website design Midsomer Norton
- website design Radstock
- website design Peasedown St John
- website design Paulton
- website design Saltford
- website design Timsbury

**Nearby (e.g. Trowbridge, Wiltshire)**
- website design Trowbridge
- web design Trowbridge
- website design Somerset
- web design Somerset
- website design Wiltshire

**Combined**
- website design Bath and BANES
- website design for trades Bath
- cheap website design Bath
- business website design Bath

---

## 7b. Related website design keywords (beyond “website design [location]”)

Use these on /web-design, /web-design-packages, blog posts, and GBP. Weave into headings and body copy where natural.

**Offer / format**
- one-page website | single page website
- website from £399 | affordable website Bath
- small business website | business website Bath
- website for trades | website for tradesmen
- website for local business

**Outcome / benefit**
- website that ranks | SEO website
- fast website | fast loading website
- mobile-friendly website
- website that gets found on Google

**Alternative search terms**
- web design packages Bath
- website builder Bath (some users search “builder” not “design”)
- get a website | new website for business
- professional website Bath

**Long-tail**
- affordable website design for small business Bath
- one-page website for trades Bath
- website design for plumbers electricians Bath

---

## 8. How to use this list

- **Homepage / service pages:** Weave in brand + service keywords (local SEO Bath, Google Business Profile BANES, free audit).
- **Area pages:** Use that area’s block from section 4 in meta title, meta description, H1/H2, and body copy.
- **Web Design page:** Use section 7b (related website design keywords) in one H2/FAQ and meta where natural. Use section 7 (website design + location) in meta title, description, and an “Areas we serve” section (Bath, Keynsham, Trowbridge, etc.).
- **GBP / ads / posts:** Use area + trade phrases and “free audit”, “BANES”, “map pack”.
- **Blog / tips:** Use trade + area and “Google Business Profile”, “local SEO”, “reviews”, “map pack”.
- **Internal linking:** Link area pages to each other and to services using these phrases where natural.

Update this file when you add new areas or trades.
