# Google Business Profile – Services Descriptions

Add these services in your GBP under **Edit profile → Services**. Use the **Service name** and **Description** exactly as below. **Limit: 120 characters per description.**

---

## 1. Local SEO

**Service name:** Local SEO  
**Description (for GBP):**
```
Rank on Google when Bath & BANES customers search. Keyword research, on-page optimisation, local links, monthly reports.
```
*Under 120 characters*

---

## 2. Google Business Profile Management

**Service name:** Google Business Profile Management  
**Description (for GBP):**
```
We optimise and manage your GBP so you appear in the map pack. More calls, reviews, posts. Bath & BANES.
```
*97 characters*

---

## 3. Website Design

**Service name:** Website Design  
**Description (for GBP):**
```
Fast, mobile-friendly websites for Bath & BANES. SEO-ready, conversion-focused. Sites that rank on Google.
```
*88 characters*

---

## 4. GBP Setup (optional – if you offer one-time setup)

**Service name:** Google Business Profile Setup  
**Description (for GBP):**
```
One-time GBP setup: claim, verify, add categories, hours, photos. Get visible in the map pack. Bath & BANES.
```
*95 characters*

---

## 5. Free Local SEO Audit (optional – lead magnet)

**Service name:** Free Local SEO Audit  
**Description (for GBP):**
```
Free audit: we check your GBP, website and local rankings for Bath & BANES. Clear report, no obligation.
```
*92 characters*

---

## How to add in Google Business Profile

1. Open [Google Business Profile](https://business.google.com) and select your business.
2. Click **Edit profile** (or **Profile** → **Edit profile**).
3. Go to **Services** (or **Services** in the menu).
4. Click **Add service** (or **Add a service**).
5. Enter **Service name** and **Description** from the table above.
6. Save. Repeat for each service you offer.

Use services **1, 2 and 3** as your core list. Add **4** if you sell GBP setup separately, and **5** if you want the free audit as a visible service.
