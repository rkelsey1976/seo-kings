# Google Business Profile – Business description

**Where to paste this:** Google Business Profile → Edit profile → Business description (750 character limit).

---

## Current description (website-design focused)

Copy the text below into your GBP:

```
SEO Kings is a Bath-based website design and local SEO agency for tradespeople and small businesses across Bath and North East Somerset (BANES). We build fast, professional websites and get you found on Google — website design from £250, plus Google Business Profile optimisation and local SEO so you rank in your area. We work with plumbers, electricians, roofers, cleaners and other local service businesses. Website designer in Bath, Keynsham, Midsomer Norton, Radstock and BANES. We've helped over 150 local businesses improve their visibility and grow their enquiries. No jargon, clear reports, honest pricing. Free local SEO audit. Based in Bath, serving BANES.
```

**Character count:** ~428 (under 750)

**Target keywords included:** website design, website designer, Bath, BANES, Keynsham, Midsomer Norton, Radstock, local SEO, Google Business Profile, plumbers, electricians, roofers, free local SEO audit.

---

*Source of truth in code: `src/constants/business.js` → `GBP_DESCRIPTION`*
